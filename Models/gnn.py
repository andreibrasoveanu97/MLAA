...
class GNN(torch.nn.Module):
    ...
    def __init__(self, num_classes, num_tasks, num_layer = 5, emb_dim = 300, 
                    ...
                    node_encoder = lambda x: x, edge_encoder = lambda x: x, use_node_encoder = True, graph_features = 0, num_mlp_layers = 1, global_index = "None"):
        '''
        ...
        if self.global_index == "wiener":
            self.g_index = nx.wiener_index

        ### Add global index to the graph
        print(f"graph_global_index: {global_index}")
    ...
    def forward(self, batched_data):
        ...
        # attach a global index
        batch_graphs = batched_data.to_data_list()

        if(self.global_index == "wiener"):
            batch_gis = torch.tensor(list(map(self.g_index, list(map(utils.to_networkx, batch_graphs)))))
            h_graph = torch.cat([h_graph, batch_gis.reshape(-1, 1)], dim=1)
        elif(self.global_index == "hosoya"):
            batch_gis = torch.tensor([len(y) for y in list(
                map(self.hosoya, [x.to_undirected() for x in (list(map(utils.to_networkx, batch_graphs)))]))])
            h_graph = torch.cat([h_graph, batch_gis.reshape(-1, 1)], dim=1)
        elif(self.global_index == "firstEigen"):
            batch_gis = torch.tensor(np.array([y[0] for y in list(map(nx.laplacian_spectrum, [x.to_undirected() for x in
                                                                                              (list(
                                                                                                  map(utils.to_networkx,
                                                                                                      batch_graphs)))]))])).float()
            h_graph = torch.cat([h_graph, batch_gis.reshape(-1, 1)], dim=1)
        elif (self.global_index == "secondEigen"):
            batch_gis = torch.tensor(np.array([y[1] for y in list(map(nx.laplacian_spectrum, [x.to_undirected() for x in
                                                                                              (list(
                                                                                                  map(utils.to_networkx,
                                                                                                      batch_graphs)))]))])).float()
            h_graph = torch.cat([h_graph, batch_gis.reshape(-1, 1)], dim=1)
