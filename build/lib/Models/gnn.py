...
class GNN(torch.nn.Module):
    ...
    def __init__(self, num_classes, num_tasks, num_layer = 5, emb_dim = 300, 
                    ...
                    node_encoder = lambda x: x, edge_encoder = lambda x: x, use_node_encoder = True, graph_features = 0, num_mlp_layers = 1, global_index = "firstEigen"):
        '''
    ...
    def forward(self, batched_data):
        ...
        if(self.global_index == "wiener"):
            ...
            batch_gis = torch.tensor([y[0] for y in list(map(nx.laplacian_spectrum, [x.to_undirected() for x in (list(map(utils.to_networkx, batch_graphs)))]))]).float()
            h_graph = torch.cat([h_graph, batch_gis.reshape(-1, 1)], dim=1)
    ...
    def set_mlp(self, graph_features = 0, copy_emb_weights = False):
        ...
        for i in range(self.num_mlp_layers):
            in_size = hidden_size if i > 0 else self.emb_dim + graph_features + 1
